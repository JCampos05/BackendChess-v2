...
const getEstadisticasGenerales = async (params) => {
    ...
    for (const i of inscripciones) {
        ...
        if (pagado >= costo && costo > 0)
            pagosCompletos++;
        else if (pagado > 0 && pagado < costo)
            pagosParciales++;
        else
            sinPago++;
        if (pagado > 0) {
            sumaPagosConMonto += pagado;
            contadorConMonto++;
        }
    }
    ...
    return {
        ...
        porcentaje_recaudacion: totalEsperado > 0
...
const getEstadisticasPorCategoria = async (params) => {
    const where = buildWhere(params);
    // Categorías únicas con inscripciones
    const categorias = await database_1.default.categoria.findMany({
...
const getEstadisticasPorTorneo = async (params) => {
    const torneoWhere = params.idTorneo
...
// ── Evolución temporal ───────────────────────────────────────
const getEvolucionTemporal = async (params) => {
    ...
    for (const i of inscripciones) {
        ...
        if (pagado >= costo && costo > 0)
            entry.pagos_completos++;
        else if (pagado > 0 && pagado < costo)
            entry.pagos_parciales++;
        mapa.set(key, entry);
    }
...
// ── Comparativa anual ────────────────────────────────────────
const getComparativaAnual = async (params) => {
    const where = params.idTorneo
        ? { idTorneo: params.idTorneo }
        : {};
    const inscripciones = await database_1.default.inscripcion.findMany({
        where,
    ...
    for (const i of inscripciones) {
        ...
        entry.total_recaudado += pagado;
        if (pagado > 0) {
            entry.suma_pagos += pagado;
            entry.count_con_pago++;
        }
...
//# sourceMappingURL=estadisticas-pago.service.js.map