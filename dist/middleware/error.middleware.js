...
exports.errorHandler = exports.CorsError = exports.ValidationError = exports.ConflictError = exports.ForbiddenError = exports.UnauthorizedError = exports.NotFoundError = exports.AppError = void 0;
...
class CorsError extends AppError {
    constructor(origin) {
        super(origin ? `Origen no permitido: ${origin}` : 'Origen no permitido por política CORS', 403, 'CORS_BLOCKED');
    }
}
...
const errorHandler = (err, _req, res, _next) => {
    ...
    if (err instanceof zod_1.ZodError) {
        const errores = err.errors.map((e) => e.path.length ? `${e.path.join('.')}: ${e.message}` : e.message);
